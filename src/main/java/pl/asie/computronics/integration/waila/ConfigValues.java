...
public enum ConfigValues {
	...
	static void registerConfigs(IWailaRegistrar reg) {
		for(ConfigValues value : ConfigValues.values()) {
			if(value.modID == null || Loader.isModLoaded(value.modID) || Mods.API.hasAPI(value.modID)) {
	...
	public boolean getValue(IWailaConfigHandler config) {
		return (this.modID == null || Loader.isModLoaded(this.modID) || Mods.API.hasAPI(this.modID))
