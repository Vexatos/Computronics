...
public class GuiTapePlayer extends GuiContainer {
	...
	public int xCenter;
	public int yCenter;
	public final ContainerTapeReader container;

	public GuiTapePlayer(ContainerTapeReader container) {
		super(container);
		String textureName = "computronics:tape_player";
		int xSize = 176, ySize = 166;
	...
	public void setState(State state) {
		if(this.container.getHandler().exists()) {
			try {
				Packet packet = Computronics.packet.create(this.container.getHandler().getSyncPacketID());
				this.container.getHandler().writeLocation(packet);
				packet.writeByte((byte) state.ordinal());
				Computronics.packet.sendToServer(packet);
				this.container.getHandler().switchState(state);
			} catch(Exception e) {
				//NO-OP
			}
		}
	...
	public void updateScreen() {
		super.updateScreen();
		this.state = this.container.getHandler().getEnumState();
	}
