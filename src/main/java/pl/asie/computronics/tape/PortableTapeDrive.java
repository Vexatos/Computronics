	...
	public void onInvUpdate() {
		if(this.inventory == null) {
			if(state.getStorage() != null) { // Tape was inserted
				// Play eject sound
				world.playSound(null, pos.xCoord, pos.yCoord, pos.zCoord, Sounds.TAPE_EJECT.event, SoundCategory.BLOCKS, 1, 0);
			}
			unloadStorage();
		} else {
			loadStorage();
			if(this.inventory.getItem() instanceof IItemTapeStorage) {
				// Play insert sound
				world.playSound(null, pos.xCoord, pos.yCoord, pos.zCoord, Sounds.TAPE_INSERT.event, SoundCategory.BLOCKS, 1, 0);
			}
		}
		save(getTag());
	}
