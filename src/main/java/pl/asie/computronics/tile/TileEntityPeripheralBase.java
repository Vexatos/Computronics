...
public abstract class TileEntityPeripheralBase extends TileMachine implements Environment, IPeripheral, INedoPeripheral {
    ...
    public void readFromNBT(final NBTTagCompound nbt) {
		super.readFromNBT(nbt);
		if(Loader.isModLoaded(Mods.OpenComputers)) readFromNBT_OC(nbt);
		if(Loader.isModLoaded(Mods.NedoComputers)) readFromNBT_NC(nbt);
	}

	@Override
    public void writeToNBT(final NBTTagCompound nbt) {
		super.writeToNBT(nbt);
		if(Loader.isModLoaded(Mods.OpenComputers)) writeToNBT_OC(nbt);
		if(Loader.isModLoaded(Mods.NedoComputers)) writeToNBT_NC(nbt);
	}
}
