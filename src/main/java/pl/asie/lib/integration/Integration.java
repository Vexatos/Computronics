...
public class Integration {
	 ...
	 */
	public static boolean isTool(ItemStack stack, EntityPlayer player, BlockPos pos) {
		for(IToolProvider provider : toolRegistry) {
			try {
				if(provider.isTool(stack, player, pos)) {
					return true;
				}
			} catch(Exception e) {
	 ...
	 */
	public static boolean useTool(ItemStack stack, EntityPlayer player, BlockPos pos) {
		for(IToolProvider provider : toolRegistry) {
			try {
				if(provider.useTool(stack, player, pos)) {
					return true;
				}
			} catch(Exception e) {
