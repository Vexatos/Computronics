...
public abstract class GT_MetaTileEntity_BasicGenerator extends GT_MetaTileEntity_BasicTank {
	...
	@Override public long maxEUOutput()								{return getBaseMetaTileEntity().isAllowedToWork()?GregTech_API.VOLTAGES[mTier]:0;}
	@Override public long maxEUStore()								{return Math.max(getEUVar(), GregTech_API.VOLTAGES[mTier] * 20 + getMinimumStoredEU());}
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    	if (aBaseMetaTileEntity.isServerSide() && aBaseMetaTileEntity.isAllowedToWork() && aTick%10==0) {
    		if (mFluid == null) {
    			...
    			if (tFuelValue > 0) while (aBaseMetaTileEntity.getUniversalEnergyStored() < (maxEUOutput() * 20 + getMinimumStoredEU()) && mFluid.amount >= consumedFluidPerOperation()) {
        			if (aBaseMetaTileEntity.increaseStoredEnergyUnits(tFuelValue, true)) mFluid.amount-=consumedFluidPerOperation();
    			}
    		}
    		if (mInventory[getInputSlot()] != null && aBaseMetaTileEntity.getUniversalEnergyStored() < (maxEUOutput() * 20 + getMinimumStoredEU()) && GT_Utility.getFluidForFilledItem(mInventory[getInputSlot()]) == null) {
    ...
    public int consumedFluidPerOperation() {
    	return 1;
    }
    ...
    public int getFuelValue(FluidStack aLiquid) {
    	...
    	FluidStack tLiquid;
    	List<GT_Recipe> tRecipeList = getRecipes().mRecipeList;
    	if (tRecipeList != null) for (GT_Recipe tFuel : tRecipeList) if ((tLiquid = GT_Utility.getFluidForFilledItem(tFuel.getRepresentativeInput(0))) != null) if (aLiquid.isFluidEqual(tLiquid)) return (int)(((long)tFuel.mSpecialValue * getEfficiency() * consumedFluidPerOperation()) / 100);
    	return 0;
    }
    
    public int getFuelValue(ItemStack aStack) {
    	if (GT_Utility.isStackInvalid(aStack) || getRecipes() == null) return 0;
    	GT_Recipe tFuel = getRecipes().findRecipe(false, Long.MAX_VALUE, null, aStack);
	...
	public boolean allowPutStack(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, byte aSide, ItemStack aStack) {
		return super.allowPutStack(aBaseMetaTileEntity, aIndex, aSide, aStack) && (getFuelValue(aStack) > 0 || getFuelValue(GT_Utility.getFluidForFilledItem(aStack)) > 0);
	}
