...
public abstract class GT_MetaTileEntity_BasicMachine extends GT_MetaTileEntity_BasicTank {
	...
	protected void calculateOverclockedNess(int aEUt, int aDuration) {
	    if (aEUt <= 16) {
		    ...
		    while (mEUt <= GregTech_API.VOLTAGES[mTier-1] && mMaxProgresstime > 1) {
		    	mEUt *= 4;
		    	mMaxProgresstime /= 2;
		    }
	    }
    ...
    public int checkRecipe() {
    	...
    	GT_Recipe tRecipe = tMap.findRecipe(getBaseMetaTileEntity(), mLastRecipe, false, GregTech_API.VOLTAGES[mTier], new FluidStack[] {getFillableStack()}, getAllInputs());
