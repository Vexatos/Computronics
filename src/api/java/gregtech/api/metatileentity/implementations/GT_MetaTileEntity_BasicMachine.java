...
public abstract class GT_MetaTileEntity_BasicMachine extends GT_MetaTileEntity_BasicTank {
	public boolean mAllowInputFromOutputSide = false, mFluidTransfer = false, mOutputBlocked = false, mItemTransfer = false, mHasBeenUpdated = false, mStuttering = false, mCharge = false, mDecharge = false;
	 ...
	 */
	public GT_MetaTileEntity_BasicMachine(int aID, String aName, String aNameRegional, int aTier, String aDescription, int aInputSlotCount, int aOutputSlotCount, String aGUIName, String aNEIName, ITexture... aOverlays) {
	...
	public GT_MetaTileEntity_BasicMachine(String aName, int aTier, String aDescription, ITexture[][][] aTextures, int aInputSlotCount, int aOutputSlotCount, String aGUIName, String aNEIName) {
	...
	@Override public long getMinimumStoredEU()						{return GregTech_API.VOLTAGES[mTier]*16;}
	@Override public long maxEUStore()								{return GregTech_API.VOLTAGES[mTier]*64;}
    @Override public long maxEUInput()								{return GregTech_API.VOLTAGES[mTier];}
    @Override public long maxSteamStore()							{return maxEUStore();}
    @Override public long maxAmperesIn()							{return (mEUt * 2) / GregTech_API.VOLTAGES[mTier] + 1;}
	...
	@Override public boolean isFluidInputAllowed(FluidStack aFluid)	{return getRecipeList() != null && getRecipeList().containsInput(aFluid);}
	...
	public void loadNBTData(NBTTagCompound aNBT) {
		...
		for (int i = 0; i < mOutputItems.length; i++) {
            NBTTagCompound tNBT = aNBT.getCompoundTag("mOutputItem"+i);
	...
	public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
	    ...
	    if (aBaseMetaTileEntity.isServerSide()) {
			...
			if (mMainFacing < 2 && aBaseMetaTileEntity.getFrontFacing() > 1) {
				mMainFacing = aBaseMetaTileEntity.getFrontFacing();
			}
			if (mMainFacing >= 2 && !mHasBeenUpdated) {
				mHasBeenUpdated = true;
				aBaseMetaTileEntity.setFrontFacing(aBaseMetaTileEntity.getBackFacing());
			}
	    	...
	    	if (mMaxProgresstime > 0 && (mProgresstime >= 0 || aBaseMetaTileEntity.isAllowedToWork())) {
	    		...
	    		if (mProgresstime < 0 || aBaseMetaTileEntity.decreaseStoredEnergyUnits(mEUt, false)) {
			...
			if (mFluidTransfer && getDrainableStack() != null && aBaseMetaTileEntity.getFrontFacing() != mMainFacing && (tSucceeded || aTick%20 == 0)) {
			...
			if (doesAutoOutput() && !isOutputEmpty() && aBaseMetaTileEntity.getFrontFacing() != mMainFacing && (tSucceeded || mOutputBlocked || aBaseMetaTileEntity.hasInventoryBeenModified() || aTick%600 == 0)) {
			...
			if (mOutputBlocked && isOutputEmpty()) mOutputBlocked = false;
			
	    	if (allowToCheckRecipe()) {
	    		if (mMaxProgresstime <= 0 && aBaseMetaTileEntity.isAllowedToWork() && (tRemovedOutputFluid || tSucceeded || aBaseMetaTileEntity.hasInventoryBeenModified() || aTick%600 == 0 || aBaseMetaTileEntity.hasWorkJustBeenEnabled()) && aBaseMetaTileEntity.isUniversalEnergyStored(getMinimumStoredEU() / 2)) {
	    			if (checkRecipe() == 2) {
	...
	protected void calculateOverclockedNess(GT_Recipe aRecipe) {
	    if (aRecipe.mEUt <= 16) {
		    ...
		    while (mEUt <= GregTech_API.VOLTAGES[mTier-1]) {
		    	mEUt *= 4;
		    	mMaxProgresstime /= 2;
		    }
	    }
	...
	protected boolean canOutput(GT_Recipe aRecipe) {
		return aRecipe != null && canOutput(aRecipe.getFluidOutput(0)) && canOutput(aRecipe.mOutputs);
	}
	...
	public boolean allowCoverOnSide(byte aSide, GT_ItemStack aCoverID) {
		return aSide!=getBaseMetaTileEntity().getFrontFacing()&&(aSide!=mMainFacing||GregTech_API.getCoverBehavior(aCoverID.toStack()).isGUIClickable(aSide, GT_Utility.stackToInt(aCoverID.toStack()), 0, getBaseMetaTileEntity()));
	}
    ...
    public int checkRecipe() {
    	...
    	GT_Recipe tRecipe = tMap.findRecipe(mLastRecipe, false, GregTech_API.VOLTAGES[mTier], new FluidStack[] {getFillableStack()}, getAllInputs());
