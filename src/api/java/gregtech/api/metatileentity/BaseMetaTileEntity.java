...
public class BaseMetaTileEntity extends BaseTileEntity implements IGregTechTileEntity {
	...
	@Override public void setInventorySlotContents(int aIndex, ItemStack aStack) {mInventoryChanged = true; if (canAccessData()) mMetaTileEntity.setInventorySlotContents(aIndex, GT_OreDictUnificator.setStack(true, aStack));}
    ...
    public void doExplosion(long aAmount) {
    	if (canAccessData()) {
        	...
        	if (GregTech_API.sMachineWireFire && mMetaTileEntity.isElectric()) {
    	        try {
    	        	mReleaseEnergy = true;
    	        	IEnergyConnected.Util.emitEnergyToNetwork(GregTech_API.VOLTAGE_INSANE, Math.max(1, getStoredEU() / GregTech_API.VOLTAGE_INSANE), this);
		...
		if (isServerSide()) {
			if (!privateAccess() || aPlayer.getDisplayName().equalsIgnoreCase(getOwnerName())) {
				...
				if (tCurrentItem != null) {
					...
					if (GregTech_API.sWrenchList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sWrenchList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
					...
					if (GregTech_API.sScrewdriverList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sScrewdriverList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
					...
					if (GregTech_API.sHardHammerList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sHardHammerList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
					...
					if (GregTech_API.sSoftHammerList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sSoftHammerList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
						if (GT_ModHandler.damageOrDechargeItem(tCurrentItem, 1, 1000, aPlayer)) {
					...
					if (GregTech_API.sSolderingToolList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sSolderingToolList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
					...
					if (getCoverIDAtSide(aSide) == 0) {
						...
						if (GregTech_API.sCrowbarList.contains(new GT_ItemStack(tCurrentItem)) || GregTech_API.sCrowbarList.contains(new GT_ItemStack(GT_Utility.copyMetaData(GregTech_API.ITEM_WILDCARD_DAMAGE, tCurrentItem)))) {
							if (GT_ModHandler.damageOrDechargeItem(tCurrentItem, 1, 1000, aPlayer)) {
